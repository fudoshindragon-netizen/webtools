/**
 * 8P2 Electrical Controls & Power — single-page study app.
 * Loads content from content.json, renders topics + questions,
 * and persists progress in localStorage. No backend, no API keys.
 *
 * Reuses the OChem app architecture; the only material change is the
 * diagram system (see diagramBox below) — electrical diagrams are
 * original SVG / HTML tables / monospace text, NOT SmilesDrawer.
 */

const STORAGE_KEY = '8p2-practice-progress-v1';
...
// ─── Rendering: diagrams (SVG / table / text) ──────────────────
// Unlike the OChem app (which rendered molecules from SMILES via
// SmilesDrawer), this app renders author-supplied diagrams directly:
//   { type: "svg",   svg: "<svg …>", caption? }  → inline SVG (original art)
//   { type: "table", headers?, rows, caption? }  → HTML table
//   { type: "text",  code: "…", caption? }       → monospace block (formulas)
// `svg` is trusted author content from our own content.json (not user input).
function diagramBox(diagram) {
  if (!diagram || !diagram.type) return '';
  let body = '';
  if (diagram.type === 'svg' && diagram.svg) {
    body = `<div class="diagram-svg">${diagram.svg}</div>`;
  } else if (diagram.type === 'table' && Array.isArray(diagram.rows)) {
    const headers = Array.isArray(diagram.headers) && diagram.headers.length
      ? `<thead><tr>${diagram.headers.map((h) => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead>`
      : '';
    const rows = diagram.rows
      .map((r) => `<tr>${r.map((c) => `<td>${escapeHtml(c)}</td>`).join('')}</tr>`)
      .join('');
    body = `<div class="diagram-table"><table>${headers}<tbody>${rows}</tbody></table></div>`;
  } else if ((diagram.type === 'text' || diagram.type === 'formula') && diagram.code) {
    body = `<pre class="diagram-text">${escapeHtml(diagram.code)}</pre>`;
  } else {
    return '';
  }
  const caption = diagram.caption ? `<div class="diagram-caption">${escapeHtml(diagram.caption)}</div>` : '';
  return `<div class="diagram-box">${body}${caption}</div>`;
}
